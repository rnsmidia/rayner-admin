// api/verify.js — DarkFlow License Verification
...
export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/licenses?key=eq.${encodeURIComponent(key.trim())}&select=id,key,active,email,note`,
      {
        headers: {
          'apikey':        SUPABASE_KEY,
    ...
    if (!license.active) {
      return res.status(200).json({ valid: false, error: 'Licença desativada' });
    }
